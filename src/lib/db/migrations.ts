/**
 * Embedded migrations — the source of truth for the schema.
 *
 * The SQL is embedded as a string constant so booting never depends on
 * `src/lib/db/migrations/*.sql` existing on disk at runtime (standalone
 * Docker images may not carry the source tree). On-disk .sql files in
 * that folder are still picked up as extras for forward extensibility,
 * but the embedded list guarantees a fresh deployment always creates
 * every table.
 */
